...
function buildYtDlpArgs(videoIdOrUrl, options) {
    if (!options) options = {};
    var args = ["--no-warnings", "--no-playlist", "--newline", "--progress"];
...
function isFileCollisionError(message) {
    return /already exists|cannot create a file when that file already exists|winerror 183/i.test(String(message || ""));
}
function isUnavailableFormatError(message) {
    return /requested format is not available/i.test(String(message || ""));
}
function buildAttemptConfigs(headers) {
    var list = [
        { format: "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best", cookie: headers.cookie || "" },
        { format: "bv*+ba/b", cookie: headers.cookie || "" }
    ];
    if (headers.cookie) {
        list.push(
            { format: "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best", cookie: "" },
            { format: "bv*+ba/b", cookie: "" }
        );
    }
    return list;
}
...
function createYouTubeDownloadTask(videoIdOrUrl, headers, hooks, options) {
    ...
    var retryBaseName = options.preferredName || path.parse(outputFileName).name;
    var attemptCount = 0;
    var attemptConfigs = buildAttemptConfigs(headers);
    var attemptIndex = 0;
    var promise = new Promise(function (resolve, reject) {
        ...
        function runAttempt() {
            attemptCount += 1;
            var currentAttempt = attemptConfigs[attemptIndex];
            var args = buildYtDlpArgs(videoIdOrUrl, {
                cookie: currentAttempt.cookie,
                referer: headers.referer,
                userAgent: headers.userAgent,
                outputPath: outputPath,
                format: currentAttempt.format
            });
            child = execFile(YT_DLP_BIN, args, {
                ...
                if (error) {
                    ...
                    if (isFileCollisionError(message) && attemptCount < 3) {
                        removeRelatedArtifacts(outputPath);
                        outputFileName = createSafeOutputName(DOWNLOADS_DIR, retryBaseName, 200);
                        outputPath = path.join(DOWNLOADS_DIR, outputFileName);
                        return runAttempt();
                    }
                    if (isUnavailableFormatError(message) && attemptIndex < attemptConfigs.length - 1) {
                        removeRelatedArtifacts(outputPath);
                        attemptIndex += 1;
                        return runAttempt();
                    }
...
function isYouTubePlaylistUrl(url) {
    ...
    try {
        ...
        if (!/^(www\.)?(youtube\.com|music\.youtube\.com)$/i.test(parsed.hostname)) return false;
